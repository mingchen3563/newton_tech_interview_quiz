...
@GenerateMocks([RemoteV3DataSource])
void main() {
  late RemoteV3DataSource remoteV3DataSource;
  late SpaceXRepositoryImpl spaceXRepositoryImpl;
  setUp(() {
    remoteV3DataSource = MockRemoteV3DataSource();
    spaceXRepositoryImpl = SpaceXRepositoryImpl(remoteV3DataSource: remoteV3DataSource);
  });
  test('get launches empty list...', () async {
    when(remoteV3DataSource.getLaunches()).thenAnswer((_) async => []);
    final launches = await spaceXRepositoryImpl.getLaunches();
    expect(launches, isA<List<LaunchesDto>>());
  });
  test('get launches with data...', () async {
    when(remoteV3DataSource.getLaunches()).thenAnswer((_) async => [
      {
        "flight_number": 110,
        "mission_name": "CRS-21", 
